use crate::{flow::GameTick, network::{Local, Remote}};
use super::{Rigidbody};
    ...
    for (mut transform, local_body, remote_body) in query.iter_mut() {
        ...
        if game_tick.frame() > remote_body.last_updated() + 1 {
            ...
            if game_tick.frame() > remote_body.last_updated() + 2 {
                bevy::log::warn!(
                    "Remote body is behind by {} ticks",
                    game_tick.frame() - remote_body.last_updated()
                );
            }
        } else {
            ...
            if dist > INTERPOLATION_THRESHOLD {
                ...
                target_translation = Vec2::lerp(
                    local_body.position,
                    remote_body.position,
                    INTERPOLATION_SPEED,
                );
            } else {
