use crate::{flow::GameTick, physics::Rigidbody, network::{Remote, Local}};
use bevy::prelude::*;
    ...
    for (mut transform, local_body, remote_body) in query.iter_mut() {
        ...
        if local_body.is_some() {//&& remote_body.is_none() {
            ...
            transform.translation =
                Vec3::new(local_body.position.x, local_body.position.y, 1.0);
        ...
        if remote_body.is_some() && local_body.is_none() {
            ...
            transform.translation =
                Vec3::new(remote_body.position.x, remote_body.position.y, 1.0);
        ...
        if game_tick.frame() > remote_body.last_updated() {
            ...
            let dist = local_body
                .position
                .distance_squared(remote_body.position);
            if dist > 1.0 {
                // Smoothly interpolate towards the correct position.
                target_translation =
                    Vec2::lerp(local_body.position, remote_body.position, 0.3);
